...
import static com.example.cinema.domain.ShowCommandError.CANCELLING_PAID_SEAT_NOT_SUPPORTED;
...
import static com.example.cinema.domain.ShowCommandError.SEAT_NOT_RESERVED;
...
public record Show(String id, String title, Map<Integer, Seat> seats, Map<String, Integer> pendingReservations) {
  ...
  private Either<ShowCommandError, ShowEvent> handleConfirmation(ConfirmReservationPayment confirmReservationPayment) {
    return pendingReservations.get(confirmReservationPayment.reservationId()).map(seatNumber ->
            seats.get(seatNumber).<Either<ShowCommandError, ShowEvent>>map(seat -> {
              if (seat.isReserved()) {
                return right(new SeatReservationPaid(id, confirmReservationPayment.reservationId(), seatNumber));
              } else {
  ...
  private Either<ShowCommandError, ShowEvent> handleCancellation(CancelSeatReservation cancelSeatReservation) {
    return pendingReservations.get(cancelSeatReservation.reservationId()).map(seatNumber ->
        seats.get(seatNumber).<Either<ShowCommandError, ShowEvent>>map(seat -> {
          if (seat.isReserved()) {
            return right(new SeatReservationCancelled(id, cancelSeatReservation.reservationId(), seatNumber));
          } else if (seat.isPaid()) {
            return left(CANCELLING_PAID_SEAT_NOT_SUPPORTED);
          } else {
            return left(SEAT_NOT_RESERVED);
          }
        }).getOrElse(left(SEAT_NOT_EXISTS))
  ...
  public Show apply(ShowEvent event) {
    return switch (event) {
      ...
      case SeatReservationPaid seatReservationPaid -> applyReservationCompleted(seatReservationPaid);
      case SeatReservationCancelled seatReservationCancelled -> applyReservationCancelled(seatReservationCancelled);
    };
  }

  private Show applyReservationCompleted(SeatReservationPaid seatReservationPaid) {
